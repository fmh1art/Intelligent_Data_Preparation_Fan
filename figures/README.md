...
直接使用作者更新的两张中文分类图PDF，采用白底、六色平面主枝、圆点末端和短论文标签。每条主枝只连接一个子方向的文献，枝内按书目年份由下向上排列；不同主枝的高度不构成统一时间轴。

| 图 | 文件 | 页码 | 分支与节点 |
|---|---|---:|---|
| 基于人工智能的数据准备（AI4DP） | [PDF](ai4dp.pdf) | 2（竖向，上） | 六条主枝，29个节点；灰色分支汇集共同基础与综述 |
| 面向人工智能的数据准备（DP4AI） | [PDF](dp4ai.pdf) | 2（竖向，下） | 六条主枝，20个节点；橙色分支为具身数据准备 |

两图共49个节点，对应正文全部50项引用，[44,45]合并为同一资源节点。源文件分别为625×345和607×343 pt，在同一竖向页面按版心宽度等比上下排列，不栅格化、不修改原文件。RoboMIND 2.0标签保留书目年份2025及预印本标记，版本链接保留在书目中。
...
检查确认两个PDF的中文总标题、12个分支标题及论文年份、引用编号与正文和节点清单对应，编译稿保留文字和矢量路径。新版RoboMIND 2.0/DROID标签已排开；图1仍有“NALIR”大小写和FM data wrangling文字跨框问题，图2仍有“Model collaps”拼写。源图按原文件插入，这些细节记录不进入论文正文或图注。
