...
- 正文8185个汉字，含章节标题和图注。LM4DP为2454字，DP4LM为2484字，具身部分1445字。
- 50项参考文献均被正文引用，Biber输出顺序与正文首次引用顺序一致，无重复书目键、未定义引用或失效图号。
- 双树图49个节点覆盖全部50项引用，文献编号与首次引用顺序、年份与书目逐项核对。本次按全图年份顺序重新安排节点，核对49个节点及14个年份刻度，计入坐标核读误差后没有跨年份倒置，详见[时间布局审校](../figures/reviews/fig00_two_research_trees_chronology.json)；最终PNG无代码叠字或插值放大，原生尺寸为1086×1448像素。PDF内嵌图像与最终PNG逐像素一致。
