...
- 两张研究分枝图共49个节点，覆盖全部50项引用；每张图六条主枝，第一张29个节点、第二张20个节点。核对论文标签、分类归属、短枝连接及枝内年份顺序，计入坐标误差后没有枝内跨年份倒置，详见[两图审校](../figures/reviews/flat_research_trees_audit.json)。两张PNG均为1774×887像素，未作代码修图或放大；PDF内嵌图像逐像素保持一致。
