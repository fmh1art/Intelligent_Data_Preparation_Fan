...
- `latexmk -xelatex -interaction=nonstopmode -halt-on-error -outdir=build main.tex`：通过，XeLaTeX与Biber完成引用解析，公式核查后生成21页A4 PDF。
...
- PDF检查：21页均含有效文字，未发现超出页面边界的文字块；公式核查后的图1位于第5页、图2位于第11页、图3位于第13页。输出文件与本次编译文件逐字节一致。
...
- `git diff --check`：通过。
- 公式专项核查：式（1）新增执行记录以承载过程约束，明确效用方向、成本量纲、可行性及随机性边界；第4页公式与第13页示例图渲染正常。原示例及729种订单/退款组合符合有限求和恒等式，表8全部派生差值和费用换算通过十进制定点核查。Auto-Tables的Hit@k按作者评测代码澄清，详见[FORMULA_AUDIT.md](FORMULA_AUDIT.md)。
